import json
import psycopg2
from psycopg2 import sql
from verify_gearbox import verify_function

database_config = {
    "user": 'postgres',
    "password": '9812376024',
    "host": '172.19.0.2',
    "port": 5432,
    "dbname": 'postgres'
}
# will take emch and gearbox_id as input
# issue i am facing is ki jaisi hi request dobara bhejta hu ik or row create ho jati h with sam e values
#the solution is ki m jo subassembly me pehle components aaenge unko m unique kr du 


def lambda_handler(event, context):
    body = json.loads(event.get("body", "{}"))
    emch_number = body["emch"]
    gearbox_id = body["gearbox_id"]
    print(emch_number, gearbox_id)

    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            verification=verify_function(gearbox_id)
            if verification[0]:

                dbquery_1 = """INSERT INTO powertrains(gearbox) 
                                SELECT id FROM gearboxes WHERE gearbox_id = (%s)"""
                cur.execute(dbquery_1, (gearbox_id,))
                conn.commit()

                dbquery_2="""UPDATE vehicles SET power_train = sub_query_1.id FROM
                (SELECT id FROM powertrains WHERE powertrains.gearbox IN
                (SELECT id FROM gearboxes WHERE gearboxes.gearbox_id = %s)) AS sub_query_1     
                WHERE vehicles.emch= %s """
                cur.execute(dbquery_2,(gearbox_id,emch_number,))
                conn.commit()

                msg = {
                    "message": "GearBox with gearbox id: %s added successfully in vehicle with following EMCH number: %s"
                    % (gearbox_id, emch_number)
                }
            else:
                msg={"message":verification[1]}

    return {"statusCode": 200, "body": json.dumps(msg)}
